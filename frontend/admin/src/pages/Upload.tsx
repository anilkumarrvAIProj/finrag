...
import { uploadDocument, api } from '../utils/api';
...
export default function Upload() {
  ...
  const onDrop = useCallback((accepted: File[], rejected: any[]) => {
    ...
    if (invalid.length > 0) toast.error(`Unsupported: ${invalid.join(', ')}`);
    if (valid.length > 0) setFiles(prev => [...prev, ...valid]);
  }, []);

  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop, accept: undefined, multiple: true, maxSize: 100 * 1024 * 1024,
  });
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      if (pendingFiles.length === 0) { toast.error('No files to upload'); return; }

      // Mark all pending as uploading so user sees all files in progress
      ...
      for (const fileItem of pendingFiles) {
        ...
        try {
          ...
          setFiles(prev => prev.map(f =>
            f.file === fileItem.file ? { ...f, status: 'done', docId: result.id } : f
          ));
          successCount++;
        } catch (err: any) {
          const msg = err.response?.data?.detail || err.message || 'Upload failed';
          setFiles(prev => prev.map(f =>
            f.file === fileItem.file ? { ...f, status: 'error', message: msg } : f
          ));
          failCount++;
        }
      }
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          PDF, Word, Excel and PowerPoint supported. Office files are auto-converted.
        </p>
      </div>
      ...
      {!fundId && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 text-xs text-amber-700">
          No fund selected — document will be searchable across all funds. Select a fund to isolate it.
        </div>
      )}
      ...
      >
        <input {...getInputProps()} />
        <UploadIcon size={32} className={clsx('mx-auto mb-3', isDragActive ? 'text-blue-500' : 'text-gray-300')} />
        <p className="text-sm font-medium text-gray-700">
          {isDragActive ? 'Drop files here' : 'Drag & drop files, or click to browse'}
        </p>
        <p className="text-xs text-gray-400 mt-2">PDF · DOCX · XLSX · PPTX · Max 100MB</p>
      </div>
          ...
          {files.map((item, i) => (
            <div key={i} className={clsx(
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.message && <p className="text-xs text-red-500 mt-0.5">{item.message}</p>}
              </div>
              {item.status === 'pending' && (
                <button onClick={(e) => { e.stopPropagation(); remove(i); }} className="text-gray-300 hover:text-red-500 transition p-1">
                  <X size={14} />
                </button>
              )}
              {item.status === 'uploading' && <Loader2 size={16} className="text-blue-500 animate-spin shrink-0" />}
      ...
      <div className="flex gap-3">
        ...
        >
          {uploadMutation.isPending
            ...
            : <><UploadIcon size={15} /> Upload {pendingCount > 0 ? `${pendingCount} file${pendingCount !== 1 ? 's' : ''}` : 'files'}</>
          }
        </button>
        {files.length > 0 && !uploadMutation.isPending && (
          <button onClick={() => setFiles([])} className="px-4 py-2.5 border border-gray-200 text-sm text-gray-600 rounded-lg hover:bg-gray-50 transition">
            Clear all
          </button>
        )}
      </div>
